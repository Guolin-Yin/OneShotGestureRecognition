...
dataInput = tf.keras.layers.Input( shape = [200,60,3] )
# Build the DANN model
featureEncoder = advObj.buildFeatureExtractor( )
actClsEncoder = advObj.buildSignClassifier( )
domainClsEncoder = advObj.buildDomainClassifier( )
grl = GradientReversalLayer( )

feature = featureEncoder( dataInput )
actCls = actClsEncoder( feature )
feature_2 = Concatenate([feature,actClsEncoder])
domainCls = domainClsEncoder( grl(feature_2))
dannModel = tf.keras.models.Model( dataInput, [ actCls, domainCls ] )
dannModel.summary( )