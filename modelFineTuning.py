...
class fineTuningModel:
    def __init__( self,nshots ):
        ...
        self.earlyStop = tf.keras.callbacks.EarlyStopping(monitor = 'val_loss', patience = 20, restore_best_weights =True)
        self.nshots = nshots
    ...
    def _getSQData( self,nshots:int ):
        ...
        _, _, x_all, y_all = testSign.getFormatedData( source = config.source )
        x = x_all[ 1250:1500 ]
        y = y_all[ 1250:1500 ]
        # if nshots == 1:
        #     '''For 25 way, 1 shot'''
    ...
    def _getDataToTesting(self,query_set,nway):
        '''
    ...
    def _getNShotsEmbedding( self,featureExtractor, Support_data):
        ...
        Sign_samples = np.arange( 0, len(Support_data), 25 )
        ...
        for i in Sign_class:
            ...
            five_shot_support_embedding.append(np.mean( featureExtractor.predict( np.asarray( five_shot_support_data ) ), axis = 0 ))
    ...
    def test( self ):
        ...
        feature_extractor, classifier = self._loadFineTunedModel( )
...
if __name__ == '__main__':
    ...
    config = getConfig( )
    nshots = 4
    fineTuningModelObj = fineTuningModel(nshots = nshots)
    fine_Tune_model = fineTuningModelObj.tuning()
    path = f'./models/fc_fineTuned_{nshots}_shot_with_Zscore.h5'
    fine_Tune_model.save_weights(path)
    config.settunedModel_path(path)
